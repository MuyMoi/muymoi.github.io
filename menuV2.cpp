...
//Aqui se utilizara un tamaño de columna adaptable en el formato de tablas
...
#define aNOTA 6
#define aDEF 3
#define ANCHO aID + aNOM + aCAR + aED + 3*aNOTA + aDEF
...
public:
  ...
  void mostrar() {
    ...
    cout << setw(aID) << ID << setw(aNOM) << nombre << setw(aCAR) << carrera << setw(aED)
    << edad << setw(aNOTA) << notas[0] << setw(aNOTA) << notas[1] << setw(aNOTA) << notas[2]
    << setw(aDEF) << def() << endl;
  }
  ...
  bool asgnrcarrera(string s) {
    ...
    if (tam > 60) {
      cout << "Muy extenso. Trate de acortar a maximo 60 caracteres\n\n";
      return false;
    }
    if (s.find(",") != -1) {
      cout << "No puede introducir comas\n\n";
  ...
  void pedirnombre() {
    cout << "Digitar nombre (maximo 60 letras): ";
...
private:
  ...
  void encabezado() { //encabezado de tabla de datos
    cout << left;
    cout << setw(aID) << "Codigo" << setw(aNOM) << "Nombre" << setw(aCAR) << "Carrera" << setw(aED)
    << "Edad" << setw(aNOTA) << "N1" << setw(aNOTA) << "N2" << setw(aNOTA) << "N3"
    << setw(aDEF) << "DEF" << endl;
  }
